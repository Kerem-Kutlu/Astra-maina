...
def banner():
    ...
    print(f'{lg}   Version: {w}1.2{lg} | Author: {w}Kerem_Kutlu{rs}\n')
...
print('\n' + info + lg + 'Banli hesaplar kontrol ediliyor...' + rs)
for a in accounts:
    phn = a[0]
    ...
    if not clnt.is_user_authorized():
        try:
            ...
            print(f'{error} {w}{phn} {r}banlanmis!{rs}')
            banned.append(a)
    for z in banned:
        accounts.remove(z)
        print(info+lg+' Banli hesaplar kaldirildi[Remove permanently using manager.py]'+rs)
    time.sleep(0.5)
    clnt.disconnect()


print(info+' Sessions oluşturldu!')
clr()
banner()
...
def log_status(scraped, index):
    with open('status.dat', 'wb') as f:
        ...
        f.close()
    print(f'{info}{lg} Session depolanmis {w}status.dat{lg}')
    

def exit_window():
    input(f'\n{cy} Çikmak için "enter"a basiniz.')
    clr()
    banner()
    sys.exit()
...
try:
    ...
    with open('status.dat', 'rb') as f:
        ...
        f.close()
        lol = input(f'{INPUT}{cy} Uye çekme islemi devam ediyor... {w}{status[0]}{lg}? [y/n]: {r}')
        if 'y' in lol:
            ...
            scraped_grp = input(f'{INPUT}{cy} Üyelerin alinacagi acik/ozel grup linkini giriniz: {r}')
            index = 0
except:
    scraped_grp = input(f'{INPUT}{cy} Üyelerin alinacagi acık/ozel grup linkini giriniz: {r}')
    index = 0
...
number_of_accs = int(input(f'{INPUT}{cy} Kullanmak istediginiz hesap sayisini giriniz: {r}'))
print(f'{info}{cy} Bir seçeneği seçiniz{lg}')
print(f'{cy}[0]{lg} Herkese acik gruba ekle')
print(f'{cy}[1]{lg} Ozel gruba ekle')
choice = int(input(f'{INPUT}{cy} Sectiginiz sikki yaziniz: {r}'))
if choice == 0:
    target = str(input(f'{INPUT}{cy} Acik grup linkini giriniz {r}'))
else:
    target = str(input(f'{INPUT}{cy} Ozel grup linkini giriniz {r}'))
print(f'{grey}_'*50)
...
with open('vars.txt', 'wb') as f:
    ...
    for ab in to_use:
        pickle.dump(ab, f)
    f.close()
sleep_time = int(input(f'{INPUT}{cy} Istek basina beklenecek sureyi giriniz{w}[{lg}Hic bekleme olmamasi icin 0{w}]: {r}'))
#print(f'{info}{lg} Joining group from {w}{number_of_accs} accounts...')
#print(f'{grey}-'*50)
print(f'{success}{lg} -- Uyeler burdan cekiliyor {w}{len(to_use)}{lg} account(s) --')
adding_status = 0
approx_members_count = 0
for acc in to_use:
    stop = index + 60
    c = TelegramClient(f'sessions/{acc[0]}', 3910389 , '86f861352f0ab76a251866059a6adbd6')
    print(f'{plus}{grey} kullanici {cy}{acc[0]}{lg} -- {cy}Starting session... ')
    c.start(acc[0])
    ...
    try:
        if '/joinchat/' in scraped_grp:
            ...
            try:
                ...
                print(f'{plus}{grey} Kullanici {cy}{acc_name}{lg} -- Uye cekmek icin gruba katildi')
        ...
        if choice == 0:
            ...
            print(f'{plus}{grey} Kullanici: {cy}{acc_name}{lg} -- Gruba eklendi')
            ...
            try:
                ...
                print(f'{plus}{grey} User: {cy}{acc_name}{lg} -- Gruba eklendi')
            except UserAlreadyParticipantError:
                pass
            target_entity = c.get_entity(target)
            target_details = target_entity
    except Exception as e:
        print(f'{error}{r} Kullanici {cy}{acc_name}{lg} -- Gruba katilmakta hata olustu')
        print(f'{error} {r}{e}')
        continue
